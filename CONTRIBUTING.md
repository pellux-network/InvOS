...
- Don't hand-edit `controller/storage/recipes/`. It's generated by
  ...
  generated pack. It's gitignored and never listed in `deployment_manifest.lua` — it's
  per-deployment data derived from one modpack's own game, not source. `tools/deploy.py`
  pushes it separately, from whatever local copy exists; a fresh clone has none until you
  regenerate one, and the controller runs fine without it, just with nothing craftable.
