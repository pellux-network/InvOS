...
- `scenario.crafting()` boots a **second** emulated computer running `turtle/`, installed from that tree's own manifest, so a whole craft runs there: real firmware, real rednet on `invos-craft`, real item movement. CraftOS-PC emulates no turtles at all, so the harness injects `turtle` as a stateless proxy (`smoke/turtle_api.lua`) forwarding to a world server beside the controller (`smoke/world_turtle.lua`) — peripherals do not cross computers in CraftOS-PC, so the turtle cannot reach a chest itself. Item moves are real `pushItems`/`pullItems` between emulated chests; only crafting creates items, and only through `smoke/craft_oracle.lua`, a five-recipe table kept deliberately independent of the recipe pack so a scenario can make the world disagree with it. The recipe pack is installed separately from the manifest, exactly as `tools/deploy.py` pushes it.
