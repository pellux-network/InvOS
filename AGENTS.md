...
- `controller/storage/recipes/` holds the generated crafting recipe pack, never hand-edited; regenerate it with `tools/recipe_import.py` and see `docs/operations.md`. It's gitignored and deployed separately from `deployment_manifest.lua`'s code allow-list, since it's per-deployment data derived from one modpack's own game, not source. Hand-written recipes go in `storage/data/custom_recipes.lua` instead, which takes precedence over it.
...
- `tools/` holds host-side build tooling that is never deployed. Its Python tests run with `python -m unittest test_recipe_pack test_recipe_import test_deploy` from `tools/`. `tools/deploy.py` is the live deployment gate; `tools/recipe_import.py` generates the recipe pack.
- `tools/emulator/` boots the controller in the CraftOS-PC emulator and drives it headlessly; see `docs/emulator.md`. It installs from `deployment_manifest.lua`, so a module missing from the manifest fails to boot there rather than in Minecraft, and it runs the Lua 5.2 that CC:Tweaked runs rather than the host's 5.4. Nothing under `controller/` or `turtle/` may require any part of it. `smoke/world.lua` uses CraftOS-PC-only calls (`periphemu`, `setItem`) and patches `getItemDetail` to return the `displayName`/`maxCount`/`tags` that CC:Tweaked returns and the emulator does not; that patch exists to match Minecraft, and the controller must never depend on it. `World.fillFirstAvailableSlot` is a second such patch and a more consequential one: CraftOS-PC's `pushItems`/`pullItems` given no target slot only merge into an existing matching stack and move **nothing** once it is full, where CC:Tweaked fills the first slot that will take them. `core/planner.lua`'s `planRetrieval` names a destination node and no slot on purpose, so without that patch any multi-stack retrieval, craft delivery or drop-off import blocks with `PICKUP_FULL` against an almost-empty Pickup -- an emulator artefact, not a controller defect.
- `scenario.crafting()` boots a **second** emulated computer running `turtle/`, installed from that tree's own manifest, so a whole craft runs there: real firmware, real rednet on `invos-craft`, real item movement. CraftOS-PC emulates no turtles at all, so the harness injects `turtle` as a stateless proxy (`smoke/turtle_api.lua`) forwarding to a world server beside the controller (`smoke/world_turtle.lua`) — peripherals do not cross computers in CraftOS-PC, so the turtle cannot reach a chest itself. Item moves are real `pushItems`/`pullItems` between emulated chests; only crafting creates items, and only through `smoke/craft_oracle.lua`, which by default loads every recipe in the installed pack (26,087 of them index in ~100ms on a real modpack) and resolves tags, so any modded item the controller can plan is one the emulator can craft. It re-checks the arrangement independently but believes the same pack, so it cannot catch a recipe the game does not have; a scenario passing an explicit recipe list makes the world disagree on purpose. The recipe pack is installed separately from the manifest, exactly as `tools/deploy.py` pushes it, and `--pack local` runs against the real generated one.
...
- Deploy with `tools/deploy.py`, which enforces the whole gate below in one command and refuses rather than guesses. `docs/operations.md` documents it, including the cross-platform path gotchas (Windows binaries invoked from WSL or Git Bash, network-mounted targets) that have caused real damage. Do not hand-roll a deployment script in a scratch directory.
- The controller does not need to be shut down for a deploy to be mechanically safe. Booting a computer loads its filesystem into the host's memory, and nothing on disk is touched again except the computer's own writes, which never land outside `storage/data/` -- a directory the deploy gate never writes to. The real hazard is racing an in-flight write to `storage/data/` (a mid-transfer journal, an in-progress config save), which is exactly what the gate's journal check and mtime-quiescence sample exist to catch. Re-read `storage/data/config.lua` before a live write regardless, to confirm the numeric ID and label are the ones you mean to target.
...
- Preserve `storage/data/`, especially `config.lua`, `aliases.lua`, `metadata.lua`, and any active journal, unless the user explicitly authorizes a fresh install. Back the directory up before deploying.
...
- **A network-mounted target (sshfs and similar) may not truncate on `open("wb")`.** Writing a file that got shorter can leave the previous version's tail in place past the new end, so the module silently stops parsing. `tools/deploy.py` unlinks before writing for exactly this reason; any other tool that writes to a live tree over such a mount must do the same. It is also why the gate verifies hashes and runs `luac -p` on what actually landed rather than trusting the write.
...
- To watch a whole craft run, `python3 tools/emulator/craftos.py craft "<query>" --count N` boots a crafting world, drives the Crafting page as an operator would, and prints the plan, the jobs list and the turtle's own screen. Add `--window turtle` to `text`/`shot` to read the turtle alone. CraftOS-PC has no tick budget, so a craft that takes minutes means the driver stopped checking, not that emulation is slow -- confirm each step changed the screen before sending the next.
...
- Merge only from a clean, fully tested branch; rerun the full host Lua suite (`lua storage/tests/run.lua`, cheap) on the merged `main` tree, plus `python3 tools/emulator/run_tests.py all`, which is cheap enough (~2 min, including a whole craft through the emulated turtle) to run on every merge rather than choosing categories by hand.
