...
- Host Lua is 5.4; CC:Tweaked runs Lua 5.2. A green host suite does not prove CC compatibility, so avoid host-only syntax and treat version-sensitive semantics with suspicion. `python3 -m unittest test_smoke` from `tools/emulator/` boots the controller under the real Lua 5.2 and is the cheapest way to prove a change actually runs; it is not a substitute for the host suite, which covers far more. The full emulator suite (`test_smoke` + `test_smoke_nbt`) takes several minutes because most of it boots a real emulator, so pick the category that covers what changed with `python3 tools/emulator/run_tests.py <category>` (`--list` to see them: `fast`, `smoke`, `setup-wizard`, `manifest`, `keys`, `nbt`, `emulator`, `all`) instead of running the whole thing every time. Never run two `run_tests.py`/emulator invocations at once — they share the fixed workdir `/tmp/invos-emulator-run` and racing corrupts both runs.
...
- Merge only from a clean, fully tested branch; rerun the full host Lua suite (`lua storage/tests/run.lua`, cheap) on the merged `main` tree, plus whichever `run_tests.py` emulator categories the merged change actually touches -- not the whole emulator suite by default, since avoiding that on every merge is the reason the categories exist. Run the full emulator suite (`python3 tools/emulator/run_tests.py all`) only when the change is broad enough, or risky enough, to warrant it.
