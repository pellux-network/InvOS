...
-- The modes F10 can pop out of, and so the only ones whose footer may advertise it. Keep
-- this in step with ui.lua's CANCEL reducer: the two together are what stop the footer
-- promising a key that quietly does nothing, which is the exact drift this module exists
-- to kill.
local POPPING_MODES = {
    quantity = true,
    variant = true,
...
M.global = {
    -- F10 pops exactly one level and is a no-op wherever there is no level to pop, so this
    -- is an allow-list of the modes that actually stack rather than a list of exclusions.
    -- Every plain page is a root now: from one, F10 does nothing and "1" is the only way
    -- back to Search. setup and setup_rename carry their own F10 entries instead, because
    -- their keymap blocks return before the shared dispatch is ever reached.
    {key="F10", label="back", detail="Go back one step", important=true,
        when=function(state)
            return POPPING_MODES[state.mode] == true
        end},
    ...
    -- Modal-only on purpose. F10 no longer falls back to jumping to Search, so a digit is
    -- the only way there -- but the nav bar already prints every page's digit on every
    -- screen for exactly that reason, so repeating them here would spend footer width on
    -- the one hint that is never hidden anyway.
...
M.per_mode = {
    ...
    search = {
        ...
        {key="Enter", label="retrieve", detail="Open the retrieve prompt for the selected item",
            important=true},
        {key="Tab", label="complete", detail="Complete the highlighted item and open its " ..
    ...
    craft_search = {
        ...
        {key="Enter", label="choose", detail="Open the quantity prompt for the selected recipe",
            important=true},
        {key="Tab", label="complete", detail="Complete the highlighted recipe and open " ..
            "its quantity prompt", important=true},
    ...
    craft_jobs = {
        -- F2 sits ahead of retry/cancel because the footer drops trailing entries first,
        -- and this is the only way out of the jobs view that the footer ever shows: F10
        -- pops here too, but it is a global and so is dropped even earlier.
        ...
        {key="F2", label="search", detail="Return to recipe search", important=true},
    ...
    page = {
        ...
        setup = {
            -- The page's whole purpose, so it leads the footer: leaving it modal-only left
            -- the Setup page advertising nothing but "P pause".
            {key="Enter", label="setup", detail="Open the full setup wizard", important=true},
        },
    },
}
