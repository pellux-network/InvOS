...
return {
    ...
    -- Retrieval scan scope. The planning gate used to rescan every storage node before every
    -- batch, which is O(installation) work for a plan that draws from one or two chests --
    -- and it is the fixed cost that made pickup feel slower the more storage was added.
    -- index:sources already names exactly the nodes holding the identity, so the gate can ask
    -- for those. Untouched nodes are safe to leave stale: reconciliation measures an aggregate
    -- per-identity delta, and a node nobody touched contributes the same amount to the
    -- baseline and to the after-total, so it cancels out.
    {name="a retrieval gates only on the storage nodes that hold the item",run=function()
        local coordinator = build({
            nodes={
                {id="storage_1",role="storage",peripheral_name="store1",priority=1},
                {id="storage_2",role="storage",peripheral_name="store2",priority=2},
                {id="storage_3",role="storage",peripheral_name="store3",priority=3},
                {id="pickup",role="pickup",peripheral_name="pickup"},
            },
            requests=requestList({
                {state="PLANNING", destination_role="pickup", requested=8, delivered=0,
                    identity={key="minecraft:diamond\0-"}},
            }),
        })
        coordinator.index = {sources=function(_, key)
            T.equal(key, "minecraft:diamond\0-")
            return {{peripheral_name="store2", count=64, slot=1}}
        end}
        local gated = names(coordinator:_preflightNames("requests"))
        T.equal(gated["store2"] or gated["storage_2"], true, "the holding node must be scanned")
        T.equal(gated["storage_1"], nil, "an unrelated storage node must not be scanned")
        T.equal(gated["storage_3"], nil, "an unrelated storage node must not be scanned")
        T.equal(gated["pickup"], true, "the destination is still gated on")
    end},
    -- The narrowing is only safe while the index can already account for the whole request.
    -- If it cannot, the shortfall may be sitting in a node whose snapshot is stale, and a
    -- narrow scan would never discover it -- the request would replan against the same
    -- incomplete picture and partial-fill in a loop.
    {name="a retrieval the index cannot fully satisfy falls back to scanning every node",
     run=function()
        local coordinator = build({
            nodes={
                {id="storage_1",role="storage",peripheral_name="store1",priority=1},
            ...
            requests=requestList({
                {state="PLANNING", destination_role="pickup", requested=100, delivered=0,
                    identity={key="minecraft:diamond\0-"}},
            }),
        })
        coordinator.index = {sources=function() return {{peripheral_name="store2", count=64}} end}
        local gated = names(coordinator:_preflightNames("requests"))
        T.equal(gated["storage_1"], true, "a shortfall must widen the scan, not narrow it")
        T.equal(gated["storage_2"], true)
    end},
    {name="a retrieval with no index falls back to scanning every storage node",run=function()
        local coordinator = build({
            nodes={
                {id="storage_1",role="storage",peripheral_name="store1",priority=1},
                {id="storage_2",role="storage",peripheral_name="store2",priority=2},
            ...
            requests=requestList({
                {state="PLANNING", destination_role="pickup", requested=1, delivered=0,
                    identity={key="minecraft:diamond\0-"}},
            }),
        })
        local gated = names(coordinator:_preflightNames("requests"))
