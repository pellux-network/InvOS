...
return {
    ...
    { name = "interrupted replacement recovers the previous valid value", run = function()
        ...
        local store = Store.new(fsApi, tokenCodec(), "storage/data")
        T.truthy(store:write("config", { schema = 1, value = "old" }, validate))
        fsApi.failMoveTo = "storage/data/config.lua"
    ...
    { name = "recovery falls back from malformed active data", run = function()
        ...
        fsApi.files["storage/data/config.previous.lua"] = fsApi.files["storage/data/config.lua"]
        fsApi.files["storage/data/config.lua"] = "malformed"
    ...
    { name = "journal retirement removes only journal variants", run = function()
        local fsApi=T.memoryFs()
        fsApi.files["storage/data/journal.lua"]="active"
        fsApi.files["storage/data/journal.previous.lua"]="previous"
        fsApi.files["storage/data/journal.staged.lua"]="staged"
        fsApi.files["storage/data/config.lua"]="config"
        local store=Store.new(fsApi,tokenCodec(),"storage/data")
        T.truthy(store:delete("journal"))
        T.equal(fsApi.exists("storage/data/journal.lua"),false)
        T.equal(fsApi.exists("storage/data/journal.previous.lua"),false)
        T.equal(fsApi.exists("storage/data/journal.staged.lua"),false)
        T.equal(fsApi.exists("storage/data/config.lua"),true)
    end },
    { name = "store can create a scoped writer without changing the original", run = function()
        ...
        T.equal(fsApi.exists("storage/data/backup.lua"), false)
    end },
}
